...
<style>
    ...
    #carouselExampleControls .carousel-inner{
        height:500px !important; /* Hapa ndipo unabadilisha */
    }
...
<section class="py-3">
    <div class="container">
        ...
        <div class="row row-cols-xl-3 row-cols-md-3 row-cols-sm-1 gx-2 gy-2">
            <?php 

            $posts = $conn->query("SELECT p.*, u.username,u.secret_code, u.avatar,c.name as `category` FROM `post_list` p inner join category_list c on p.category_id = c.id inner join `users` u where p.status = 1  and p.`delete_flag` = 0 and p.user_id = u.id order by abs(unix_timestamp(p.date_created)) desc");
