...
if(isset($_GET['id'])){
    ...
    if($qry->num_rows > 0){
        ...
        echo '<script> alert("Post ID is not recognized."); location.replace("./?page=posts");</script>'; // Corrected redirect
    }
}else{
    echo '<script> alert("Post ID is required"; location.replace("./?page=posts");</script>'; // Corrected redirect
}

// PHP logic to determine file type and URL for the main post's attached file
$post_file_url = '';
$post_file_extension = '';
$post_file_name = '';
$post_file_type = '';

if(!empty($file_path)){ // Assuming $file_path is now available from the query
    ...
    if (in_array(strtolower($post_file_extension), ['jpg', 'jpeg', 'png', 'gif', 'webp'])) {
        $post_file_type = 'image';
    } elseif (strtolower($post_file_extension) == 'pdf') {
        $post_file_type = 'pdf';
    } elseif (in_array(strtolower($post_file_extension), ['mp4', 'webm', 'ogg'])) { // Example video formats
        $post_file_type = 'video';
    } elseif (in_array(strtolower($post_file_extension), ['mp3', 'wav', 'ogg'])) { // Example audio formats
        $post_file_type = 'audio';
    } else {
        $post_file_type = 'other';
    }
}
...
<style>
    ...
    .comment-list img, .post-file-preview img {
        max-width: 100%;
        max-height: 150px; /* Smaller height for preview */
        height: auto;
        display: block;
        margin-bottom: 10px;
        object-fit: contain;
                ...
                <div class="contrain-fluid">
                    ...
                    <div style="line-height:1em" class="mb-3">
                        ...
                        <div class="py-1">
                            ...
                            <span class=""><?= $username . ' --- ' . $secret_code ?></span>
                        </div>
                    </div>
                    <div>
                        <?= $content ?>
                    ...
                    <?php if(!empty($file_path)): ?>
                        <div class="post-file-preview mt-4 text-center">
                            ...
                            <?php else: ?>
                                <i class="fa fa-file fa-5x text-secondary mb-2 file-preview-icon" data-file-url="<?= $post_file_url ?>" data-file-type="<?= $post_file_type ?>" data-file-name="<?= $post_file_name ?>"></i><br>
                                <small class="text-muted"><?= $post_file_name ?></small>
                            <?php endif; ?>
                            <div class="d-flex justify-content-center mt-2">
                                <button type="button" class="btn btn-sm btn-outline-primary me-2 view-file-btn"
                                        data-file-url="<?= $post_file_url ?>"
                                        data-file-type="<?= $post_file_type ?>"
                    ...
                    <div class="list-group comment-list mb-3 rounded-0">
                        ...
                        <div class="list-group-item list-group-item-action mb-1 border-top" style="text-align: right;">
                            ...
                            <?php if(!empty($row['file_path'])):
                                $comment_file_extension = pathinfo($row['file_path'], PATHINFO_EXTENSION);
                                $comment_file_url = base_url . $row['file_path'];
                                $comment_file_name = basename($row['file_path']);
                                $comment_file_type = '';
                                if (in_array(strtolower($comment_file_extension), ['jpg', 'jpeg', 'png', 'gif', 'webp'])) {
                                    $comment_file_type = 'image';
                                } elseif (strtolower($comment_file_extension) == 'pdf') {
                                    $comment_file_type = 'pdf';
                                } elseif (in_array(strtolower($comment_file_extension), ['mp4', 'webm', 'ogg'])) {
                                    $comment_file_type = 'video';
                                } elseif (in_array(strtolower($comment_file_extension), ['mp3', 'wav', 'ogg'])) {
                                    $comment_file_type = 'audio';
                                } else {
                                    $comment_file_type = 'other';
                                }
                            ?>
                                <div class="mt-2 text-center">
                                    <?php if ($comment_file_type === 'image'): ?>
                                        <img src="<?= $comment_file_url ?>" class="img-fluid rounded border mb-2 file-preview-image" alt="Attached Image" data-file-url="<?= $comment_file_url ?>" data-file-type="image" data-file-name="<?= $comment_file_name ?>">
                                    <?php elseif ($comment_file_type === 'pdf'): ?>
                                        <i class="fa fa-file-pdf fa-3x text-danger mb-2 file-preview-icon" data-file-url="<?= $comment_file_url ?>" data-file-type="pdf" data-file-name="<?= $comment_file_name ?>"></i><br>
                                        <small class="text-muted"><?= $comment_file_name ?></small>
                                    <?php else: ?>
                                        <i class="fa fa-file fa-3x text-secondary mb-2 file-preview-icon" data-file-url="<?= $comment_file_url ?>" data-file-type="<?= $comment_file_type ?>" data-file-name="<?= $comment_file_name ?>"></i><br>
                                        <small class="text-muted"><?= $comment_file_name ?></small>
                                    <?php endif; ?>

                                    <div class="d-flex justify-content-center mt-2">
                                        <button type="button" class="btn btn-sm btn-outline-primary me-2 view-file-btn"
                                                data-file-url="<?= $comment_file_url ?>"
                                                data-file-type="<?= $comment_file_type ?>"
                                                data-file-name="<?= $comment_file_name ?>">
                                            <i class="fa fa-eye"></i> View
                                        </button>
                                        <a href="<?= $comment_file_url ?>" download class="btn btn-sm btn-outline-success"><i class="fa fa-download"></i> Download</a>
                                    </div>
                                </div>
                            <?php endif; ?>
                        </div>
...
<script>
    ...
    $(function(){
        ...
        $('#comment').summernote({
            height:"15em",
            placeholder:"Write your text comment here",
            toolbar: [
                [ 'style', [ 'style' ] ],
        ...
        $('#comment-form').submit(function(e){
            ...
            var uploadedFile = $('#uploaded-file-input').prop('files')[0];
