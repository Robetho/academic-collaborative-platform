...
Class Master extends DBConnection {
	...
	function save_faculty(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Faculty Name already exists.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "New Faculty Name successfully saved.";
			else
				$resp['msg'] = " Faculty successfully updated.";
	...
	function delete_faculty(){
		extract($_POST);
		$del = $this->conn->query("DELETE `faculty` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Faculty successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
