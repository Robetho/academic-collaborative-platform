...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(empty($id)){
			...
			if($qry){
				...
				$code = $word.'-'.$number1.'-'.$number2;
				$sq = "UPDATE `users` SET secret_code = '$code' ORDER BY id DESC LIMIT 1";
				$this->conn->query($sq);
	...
	function registration(){
		...
		if(empty($_POST['firstname'])){
			$resp['status'] = 'failed';
			$resp['msg'] = 'Firstname Is Required.';
			return json_encode($resp);
		}
		if(empty($_POST['middlename'])){
			$resp['status'] = 'failed';
			$resp['msg'] = 'Middlename Is Required.';
			return json_encode($resp);
		}
		if(empty($_POST['lastname'])){
			$resp['status'] = 'failed';
			$resp['msg'] = 'Lastname Is Required.';
			return json_encode($resp);
		}
